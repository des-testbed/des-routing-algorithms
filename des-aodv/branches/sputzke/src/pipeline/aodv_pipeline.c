...
// ---------------------------- help functions ---------------------------------------
/*
* included Quality of Service - Parameter
* before next hop, ask QoS-Daemon via udp if next hop is feasible
* if so, pass value given by daemon to next hop
* if not, drop
*/
...
dessert_msg_t* _create_rrep(mac_addr route_dest, mac_addr route_source, mac_addr
rrep_next_hop, uint32_t destination_sequence_number, uint8_t flags, uint8_t hop_count,
metric_t initial_metric /*uint8_t qos_metric, double qos_constraint*/) {
    dessert_msg_t* msg;
    dessert_ext_t* ext;
    dessert_msg_new(&msg);
   ...
   // rrep_msg->qos_metric = qos_metric;
   // rrep_msg->qos_constraint= qos_constraint;
    return msg;
}
...
int aodv_handle_rreq(dessert_msg_t* msg, uint32_t len, dessert_msg_proc_t* proc, dessert_meshif_t* iface, dessert_frameid_t id) {
    ...
    if(proc->lflags & DESSERT_RX_FLAG_L25_DST) {
        ...
        dessert_msg_t* rrep_msg = _create_rrep(dessert_l25_defsrc, l25h->ether_shost,
        msg->l2h.ether_shost, rrep_seq_num, 0, 0, metric_startvalue);
        ...
        if(local_repair) {
            ...
            dessert_msg_t* rrep_msg = _create_rrep(l25h->ether_dhost, l25h->ether_shost,
            msg->l2h.ether_shost, our_dest_seq_num, 0, dest_hop_count,
            dest_metric);
            dessert_meshsend(rrep_msg, iface);
            dessert_msg_destroy(rrep_msg);
...
int aodv_handle_rrep(dessert_msg_t* msg, uint32_t len, dessert_msg_proc_t* proc, dessert_meshif_t* iface, dessert_frameid_t id) {
    ...
    /*
    //  double getQoSvalue (char* hostname,int serverPort, char* message)
      double val;
      nextHop;
      metric;
      constraint;
      if (val = getQoSvalue("127.0.0.1",QOSPORT,"nextHOP metric constraint") > 0) {
        add metric, val to msg passed to nextHop 
    ...
    */
        ...
        if(reverse_route_found) {
           ...
           /* fancy QoS stuff goes right here */ 
           /* create message for qos-daemon: "host metric constraint" "10.0.0.1 1 5"*/
         //  char qos_msg[64];
         //  char qos_mac[10];
         //  create_qos_message(qos_msg,sizeof(qos_msg),qos_mac,rrep_msg->qos_metric,rrep_msg->qos_constraint);
         //  qos_get_value("127.0.0.1", QOSPORT,qos_message); 
            /*--------*/
            /* check if link is feasible for QoS */
            /*--------*/
          //  if (qos_constraint > 0) {
              dessert_meshsend(msg, output_iface);
              comment = "forwarded";
          //  }
            //else { // Drop if not feasible
          //    comment = "dropped (not QoS feasible)";
            //  goto drop;
           // }
        }
        else {
