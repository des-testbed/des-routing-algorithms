...
def get_distance2line(((x1, y1), (x2, y2)), (x3, y3)):
    ...
    if c2 <= c1:
        return 1000
        #return distance((x3, y3), (x2, y2))

    n = math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1))
    return abs((x3 - x1) * (y2 - y1) - (y3 - y1) * (x2 - x1)) / n
...
def main():
    ...
    concave, convex = get_concave(points, 1)
    draw(options, concave, points, convex)

def get_concave(points, threshold):
    convex = list(convex_hull(numpy.array(points).transpose()))
    convex = [(x, y) for x, y in convex]
    concave = calc_concave2(points, convex, threshold=threshold)
...
def calc_concave2(points, convex, threshold=0.01):
