...
typedef void purge_object_t(time_t timestamp, void* object);
...
typedef struct timeslot {
    ...
    purge_object_t*				object_purger;
    struct timeslot_element*	elements_hash;
} timeslot_t;

/** Create time-slot */
int timeslot_create(timeslot_t** ts, time_t purge_timeout, purge_object_t* object_purger);

/** Remove all time-slot elements and destroy time-slot */
int timeslot_destroy(timeslot_t* ts);

/** Change pudge timeout */
int timeslot_change_pt(timeslot_t* ts, time_t purge_timeout);
...
/**Purges all objects older than timestamp - pudge_timeout from time-slot*/
