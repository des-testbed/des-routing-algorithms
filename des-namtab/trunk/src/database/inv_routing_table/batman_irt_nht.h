...
typedef struct batman_irt_nht_entry {
    ...
    batman_sw_t* sw;
    UT_hash_handle hh;
} batman_irt_nht_entry_t;

/** NextHop table */
typedef struct batman_irt_nht {
    batman_irt_nht_entry_t* entrys;
    /** the best hop entry with most SeqNum count towards destination */
    batman_irt_nht_entry_t* best_next_hop;
} batman_irt_nht_t;
...
int batman_db_inht_destroy(batman_irt_nht_t* rt_nh);

/** Adds a sequence number to one NextHop of NextHop Table */
int batman_db_inht_addseq(batman_irt_nht_t* rt_next_hop_table,
