...
int batman_db_nht_entry_create(batman_irt_nht_entry_t** rt_nh_entry_out, uint8_t ether_addr[ETH_ALEN]) {
    batman_irt_nht_entry_t* new_entry;
    batman_sw_t* new_sw;
    new_entry = malloc(sizeof(batman_irt_nht_entry_t));
...
int batman_db_nht_entry_destroy(batman_irt_nht_entry_t* rt_nh_entry) {
    batman_sw_destroy(rt_nh_entry->sw);
    free(rt_nh_entry);
    return true;
}

int batman_db_inht_create(batman_irt_nht_t** rt_nh_out) {
    batman_irt_nht_t* new_nl;
    new_nl = malloc(sizeof(batman_irt_nht_t));
...
int batman_db_inht_destroy(batman_irt_nht_t* rt_nht) {
    // delete HASH map and all its entrys
    batman_irt_nht_entry_t* current_entry;
...
int batman_db_inht_addseq(batman_irt_nht_t* rt_next_hop_table, uint8_t ether_nexthop_addr[ETH_ALEN], uint16_t seq_num) {
    batman_irt_nht_entry_t* nh_entry;
    ...
    batman_db_inht_shiftuptoseq(rt_next_hop_table, seq_num);
    return true;
}

int batman_db_inht_shiftuptoseq(batman_irt_nht_t* rt_next_hop_table, uint16_t seq_num) {
    batman_irt_nht_entry_t* nh_entry = rt_next_hop_table->entrys;
    ...
    while(nh_entry != NULL) {
        ...
        batman_irt_nht_entry_t* prev_element = nh_entry;
...
int batman_db_inht_get_totalsncount(batman_irt_nht_t* rt_next_hop_table) {
    int count = 0;
    batman_irt_nht_entry_t* curr_entry = rt_next_hop_table->entrys;
...
int batman_db_inht_report(batman_irt_nht_t* nh_table, char** str_out) {
    batman_irt_nht_entry_t* current_entry = nh_table->entrys;
    char* output;
